# -*- coding: utf-8 -*-
"""시드 확률 앙상블 판정: e5 hist12 avg(seed42, seed43) — 2026-07-11 저녁 축.

대조군 = colab_out/holdout_e5_h12.npz        (배포 hist12, seed42)
후보   = avg(seed42, seed43) 확률 평균        (s43 = 서버 학습분, in-memory 평균)

판정 지표 (probe_c와 동일 — row 단독 판정 금지):
...
⚠️ 통과해도 2×e5 fp16은 zip 1GB 초과 — 배포는 증류 경로만 (판정 시 시간 비용 감안).
"""
import os
...
S43_NPZ = Path(os.environ.get("E5_H12_S43", r"C:\dev\2026-AI-DACON\colab_out\holdout_e5_h12_s43.npz"))
GATE = 0.005

for p in (BASE_NPZ, S43_NPZ):
    if not p.exists():
        raise FileNotFoundError(f"npz 없음: {p} — 서버 scp 회수 확인")
...
p42 = np.asarray(common.align_npz_probs(BASE_NPZ, data.ids, data.y_true, actions), dtype=np.float64)
p43 = np.asarray(common.align_npz_probs(S43_NPZ, data.ids, data.y_true, actions), dtype=np.float64)
cand = (p42 + p43) / 2.0

# e5 solo 참고선 (blend 밖 단독 성능 — 시드 평균이 solo에서 이기는지도 기록)
solo42 = mf1(y, acts[p42.argmax(1)])
solo_avg = mf1(y, acts[cand.argmax(1)])
print(f"[참고] e5 solo: s42 {solo42:.5f} → avg(s42,s43) {solo_avg:.5f}  델타 {solo_avg - solo42:+.5f}")

p_base = final_pred(p42)
...
print("\n판정:", "게이트 통과 → 증류 경로 검토 (2×e5 배포 불가, D-011 LB 게이트 선행)" if ok else
