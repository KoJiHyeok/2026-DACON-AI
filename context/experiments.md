...
| 19 | 07.05 | history 유무 버킷별 blend 가중 (동료 0.7242 스킴 — 리포 notes/bucket-blend-2026-07-04.md + bucket-weights JSON) | 동료 JSON을 리그에서 우리 성분으로 평가: hist_empty=[0.75,0.5,2], hist_present=[0.75,1,0.75] | 리그 **0.72477 (+0.0075** vs flat [1,1,2] 0.71726**)**. 이득 전부 history-있음 버킷의 enc 지분 하향(0.30)에서 발생 | (동료 LB 0.7242로 간접 실측) | **채택** — #16에서 신기루로 기각했던 축이 동료 LB 실측으로 방향 확인됨. 재해석: 인코더는 첫 스텝에서만 압도적, history 있으면 과대 지분이었다. 리그·LB가 같은 방향을 가리키는 첫 레버 |
| 20 | 07.05 | 버킷 정밀화: history_presence(2버킷) → history_len3(0 / 1-4 / 5+) | script.py에 history_len3 스킴 추가 + model/bucket_weights.json: hist_0=[0.75,0.5,2], hist_1_4=[0.75,1,1], hist_5_plus=[0.75,1.5,0.75] | 리그 **0.72679** (동료 2버킷 0.72477 대비 +0.0020, flat 대비 +0.0095). 세션 해시 반반 안정성: 양쪽 절반 모두 2버킷보다 우세. 긴 history에서 stacker 지분↑이 새 축 | (제출 대기) | 제출 후보 — flat→2버킷의 리그 델타 +0.0075가 LB +0.0042로 전이됐으므로(전이율 ~0.56) 기대 LB ≈ 0.7188 + 0.0095×0.56 ≈ **0.724**. reviewer PASS, 게이트 PASS(대장 #4) |
...
| 로컬 그리드로 enc 지분 낮추기 (flat·전역) | 로컬 +0.009 신기루 | 리그 기울기가 enc 지분 축에서 LB와 반대 — 두 번 확인됨 (핸드오프 §5 + #16). ⚠️ 단 **버킷별** enc 지분 조정은 예외로 판명 (#19 동료 LB 실측) — 금지는 '전역 flat 가중'에 한정 |
