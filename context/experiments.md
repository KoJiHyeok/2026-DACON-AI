...
| 36 | 07.10 | **제3자 SOL 검증면 감사** — 저장소의 “숨은 test 세션당 1행” 가정을 로컬 리그가 반영하는가 | train 70k/9,429세션과 리그 9,969행/1,350세션을 재집계. row F1, session-inverse-weight F1, step별 F1, 세션당 1행 200회 Monte Carlo 비교 | hist12−hist6: row **+0.02150**, session-weighted **+0.01859**. step1 −0.0041 / step2 −0.0099 / step6 +0.0489 / step7 +0.0466 / step8 +0.0465. one-row/session 델타 평균 +0.01860±0.00863 | 미제출 | **검증 공백 확인.** 형제복원 LB 델타0은 가정을 지지하지만 숨은 구조의 직접 증거는 아님. Group split은 누수를 막아도 test 표본단위 일치는 보장하지 않으므로, 고정 홀드아웃을 dev로 동결하고 별도 shadow split + session/step-aware 지표 도입 권고. 상세: [reports/third_party_sol_model_audit_2026-07-10.md](reports/third_party_sol_model_audit_2026-07-10.md) |
