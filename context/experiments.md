...
| 22 | 07.06 | AU/SIM id-prefix 분리 분석 + AU 전용 linear specialist 프로브 (밤샘 task3) | `sess_au` 5,025행만으로 word/char TF-IDF + LinearSVC, 세션 Group 3-fold OOF. 리그 holdout AU 682행만 AU OOF 예측으로 라우팅, SIM은 3-way 그대로 유지 | AU OOF **0.68001**. 리그 proxy: AU `0.51381 → 0.69035`, all `0.71726 → 0.72661` (**+0.00935**) | 미제출 | **PASS — LB 게이트 후보**. R4/meta와 달리 약한 specialist를 전체 blend 위에 얹는 것이 아니라 id로 무료 판별되는 AU 분포에만 적용. calib/prior 유형도 아님. 단 AU holdout 682행·희소 클래스 support 작음 → 제출 승격 전 LB 1회 게이트 필수. 리포트: `context/night/2026-07-06/task3_report.md` |
