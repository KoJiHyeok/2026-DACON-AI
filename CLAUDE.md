...
- 병렬 작업을 시작하거나 파일을 수정하기 전에 **반드시 `context/coordination.md`를 확인**한다.
- Claude는 실험 큐·GPU·공식 기록·`submit/`·LB 제출의 단일 소유자다.
- Codex는 별도 worktree/branch에서 독립 구현·평가 도구를 만들고 main이나 Claude 소유 파일을 직접 수정하지 않는다.
- 한 파일에는 동시에 한 작성자만 둔다. 소유권 변경은 `context/coordination.md`에 먼저 기록한다.
- Codex 산출물은 commit/handoff 단위로 전달하고, Claude가 reviewer/tester 검증 후 선택적으로 cherry-pick한다.

## 절대 규칙 (제출 제약)

- 평가 서버: **T4 16GB / 3 vCPU / 12GB RAM, 오프라인** (네트워크 호출 코드 금지)
- **모델 역할 분담**: 오케스트레이션(메인 세션)은 **Opus**, 서브에이전트 model은 **Sonnet 5로 고정**. (이전 Fable 오케스트레이션 역할은 Opus로 이관 — 과거 커밋의 Fable co-author 트레일러는 사실 기록이라 재작성하지 않음)
...
- `/colab-run` — Colab 작업 안내 (검증된 전체 셀 시퀀스 — 데이터 준비·env·학습·다운로드를 항상 완결 셀로 출력)
